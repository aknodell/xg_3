...
get_min_model_weighted <- function(mat, is_goal, w) {
  set.seed(1138)
  cv <-
    glmnet::cv.glmnet(
      mat,
...
dynamic_xg_det_testing |>
  ...
  dplyr::select(gm_dt, xg_results) |>
  tidyr::unnest(xg_results) |>
  dplyr::select(gm_dt, det_score_rush_rebound_fac_turn) |>
  dplyr::mutate(
    coefs =
      purrr::map(
        det_score_rush_rebound_fac_turn,
        function(m) {
          coef(m) |>
            as.matrix() |>
            as.data.frame() |>
  ...
  dplyr::select(-c(det_score_rush_rebound_fac_turn)) |>
  tidyr::unnest(coefs) |>
  dplyr::filter(
    rowname != "(Intercept)"
    # !rowname %in% c("(Intercept)", "is_slap", "is_tip", "is_other")
  ...
  dplyr::mutate(
    xg_results =
      purrr::pmap(
        ...
        function(dt, m, basic, shot_type, shot_type_point) {
          start_time <- Sys.time()
          ...
          shots <-
            ...
            dplyr::inner_join(
              basic |>
                dplyr::select(
                  shot_x, shot_y, blockers_basic = blocker_dens
                ),
              by = c("shot_x", "shot_y")
            ) |>
            dplyr::inner_join(
              shot_type |>
                dplyr::select(
                  shot_x, shot_y, shot_type, blockers_type = blocker_dens
                ),
            ...
            dplyr::inner_join(
              shot_type_point |>
                dplyr::select(
                  shot_x, shot_y, shot_type, point_shot, blockers_point = blocker_dens
                ),
...
blocker_metrics <-
  ...
  dplyr::transmute(
    ...
    gm_dt,
    dplyr::across(
      -c(season:min, shot_blocker_data_5v5_basic:shot_blocker_data_5v5_shot_type_point),
      .fns =
        function(model) {
          purrr::pmap(
            list(
            ...
            function(m, d, basic, shot_type, shot_type_point) {
              shots <-
                training_data |>
                dplyr::filter(
                  game_date == d,
  ...
  dplyr::summarise(
    dplyr::across(
      det_score_rush_rebound_fac_turn:det_score_rush_rebound_fac_turn_point,
      .fns = function(xg) {
        sum(xg) / sum(is_goal)
      },
      .names = "{.col}_calib"
      ...
      purrr::pmap(
        ...
        function(dt, m) {
          ...
          shots <-
            ...
            dplyr::ungroup() |>
            dplyr::mutate(
              shooter_goalie_game_weights =
                (
                  (shooter_weight * goalie_weight * game_weight)
                    # min(shooter_weight * goalie_weight * game_weight)
                ),
              shooter_goalie_weights =
                (
                  (shooter_weight * goalie_weight)
                    # min(shooter_weight * goalie_weight)
                ),
              shooter_game_weights =
                (
                  (shooter_weight * game_weight)
                    # min(shooter_weight * game_weight)
                ),
              shooter_weights = ((shooter_weight)),# / min(shooter_weight)),
              goalie_game_weights =
                (
                  (goalie_weight * game_weight)
                    # min(goalie_weight * game_weight)
                ),
              goalie_weights = ((goalie_weight)), # / min(goalie_weight)),
              game_weights = ((game_weight)), # / min(game_weight))
            )
          ...
          min_score_rush_rebound_fac_turn_shooter_goalie_game <-
            get_min_model_weighted(mat, shots$is_goal, shots$shooter_goalie_game_weights)

          min_score_rush_rebound_fac_turn_shooter_goalie <-
            get_min_model_weighted(mat, shots$is_goal, shots$shooter_goalie_weights)

          min_score_rush_rebound_fac_turn_shooter_game <-
            get_min_model_weighted(mat, shots$is_goal, shots$shooter_game_weights)

          min_score_rush_rebound_fac_turn_shooter <-
            get_min_model_weighted(mat, shots$is_goal, shots$shooter_weights)

          min_score_rush_rebound_fac_turn_goalie_game <-
            get_min_model_weighted(mat, shots$is_goal, shots$goalie_game_weights)

          min_score_rush_rebound_fac_turn_goalie <-
            get_min_model_weighted(mat, shots$is_goal, shots$goalie_weights)

          min_score_rush_rebound_fac_turn_game <-
            get_min_model_weighted(mat, shots$is_goal, shots$game_weights)
          ...
          tibble::tibble(
            det_score_rush_rebound_fac_turn_shooter_goalie_game =
              list(min_score_rush_rebound_fac_turn_shooter_goalie_game),
            det_score_rush_rebound_fac_turn_shooter_goalie =
              list(min_score_rush_rebound_fac_turn_shooter_goalie),
            det_score_rush_rebound_fac_turn_shooter_game =
              list(min_score_rush_rebound_fac_turn_shooter_game),
            det_score_rush_rebound_fac_turn_shooter =
              list(min_score_rush_rebound_fac_turn_shooter),
            det_score_rush_rebound_fac_turn_goalie_game =
              list(min_score_rush_rebound_fac_turn_goalie_game),
            det_score_rush_rebound_fac_turn_goalie =
              list(min_score_rush_rebound_fac_turn_goalie),
            det_score_rush_rebound_fac_turn_game =
              list(min_score_rush_rebound_fac_turn_game),
            det_score_rush_rebound_fac_turn =
              list(min_score_rush_rebound_fac_turn),
          )
        }
      )
  )

save.image("C:/Users/aknod/Documents/xg_3/.RData")
...
weights_metrics <-
  ...
  dplyr::summarise(
    dplyr::across(
      det_score_rush_rebound_fac_turn_shooter_goalie_game:det_score_rush_rebound_fac_turn,
      .fns = function(xg) {
        sum(xg) / sum(is_goal)
      },
      .names = "{.col}_calib"
...
dynamic_xg_weights_testing |>
  ...
  dplyr::select(gm_dt, xg_results) |>
  tidyr::unnest(xg_results) |>
  dplyr::select(gm_dt, det_score_rush_rebound_fac_turn_game) |>
  dplyr::mutate(
    coefs =
      purrr::map(
        det_score_rush_rebound_fac_turn_game,
  ...
  dplyr::select(-c(det_score_rush_rebound_fac_turn_game)) |>
  tidyr::unnest(coefs) |>
  dplyr::filter(
    rowname == "(Intercept)"
    # geometry 3
    # rowname %in% c("dist_to_goalie_optimal", "avg_height", "width_at_net")
    # # shot type 3
    # rowname %in% c("is_slap", "is_tip", "is_other") |
    # # # score 1
    # # rowname %in% c("is_leading")
    # # # rush 3
    # rowname %in% c("is_rush", "rush_secs", "rush_velo") |
    # # # rebounds 5
    # rowname %in%
    #   c(
    #     "is_followup_shot", "is_reached_goalie_followup", "angle_change_velo",
    #     "is_own_followup", "followup_secs"
    #   ) |
    # # # faceoffs 2
    # rowname %in% c("faceoff_secs", "is_off_faceoff") |
    # # # turnover 3
    # rowname %in% c("is_off_turnover", "is_oz_turnover", "turnover_secs")
    # rowname %in% c("angle_change_velo", "rush_secs", "rush_velo")
    # !rowname %in% c("(Intercept)", "is_slap", "is_tip", "is_other")
  ) |>
  dplyr::mutate(
    # category =
    #   dplyr::case_when(
    #     rowname %in% c("is_slap", "is_tip", "is_other") ~ "Shot Type",
    #     rowname %in% c("is_rush", "rush_secs", "rush_velo")  ~ "Rush Shots",
    #     rowname %in%
    #       c(
    #         "is_followup_shot", "is_reached_goalie_followup", "angle_change_velo",
    #         "is_own_followup", "followup_secs"
    #       ) ~ "Follow-Up Shots",
    #     rowname %in% c("faceoff_secs", "is_off_faceoff") ~ "OZ Faceoffs",
    #     rowname %in% c("is_off_turnover", "is_oz_turnover", "turnover_secs") ~ "Turnovers"
    #   ) |>
    #   factor(
    #     levels = c("Shot Type", "Rush Shots", "Follow-Up Shots", "OZ Faceoffs", "Turnovers")
    #   ),
    # rowname =
    #   dplyr::case_when(
    #     rowname == "is_slap" ~ "Slap Shot",
    #     rowname == "is_tip" ~ "Tip/Deflected",
    #     rowname == "is_other" ~ "Backhand/Other",
    #     rowname == "is_rush" ~ "Is Rush",
    #     rowname == "rush_secs" ~ "Rush Seconds Elapsed",
    #     rowname == "rush_velo" ~ "Rush Velocity",
    #     rowname == "is_followup_shot" ~ "Is Follow-Up",
    #     rowname == "is_reached_goalie_followup" ~ "Is Reached Goalie Follow-Up",
    #     rowname == "is_own_followup" ~ "Is Own Shot Follow-Up",
    #     rowname == "followup_secs" ~ "Follow-Up Seconds Elapsed",
    #     rowname == "angle_change_velo" ~ "Angle Change Velocity",
    #     rowname == "is_off_faceoff" ~ "Is OZ Faceoff",
    #     rowname == "faceoff_secs" ~ "Faceoff Seconds Elapsed",
    #     rowname == "is_off_turnover" ~ "Is Off Turnover",
    #     rowname == "is_oz_turnover" ~ "Is OZ Turnover",
    #     rowname == "turnover_secs" ~ "Turnover Seconds Elapsed"
    #   ) |>
    #   factor(
    #     levels = c(
    #       "Slap Shot", "Tip/Deflected", "Backhand/Other", "Is Rush",
    #       "Rush Velocity", "Rush Seconds Elapsed", "Is Follow-Up",
    #       "Is Reached Goalie Follow-Up", "Is Own Shot Follow-Up",
    #       "Angle Change Velocity", "Follow-Up Seconds Elapsed",
    #       "Is OZ Faceoff", "Faceoff Seconds Elapsed", "Is Off Turnover",
    #       "Is OZ Turnover", "Turnover Seconds Elapsed"
    #     )
    #   ),
    dt = dplyr::dense_rank(gm_dt)
  ) |>
  # View()
  # tidyr::pivot_wider(
  #   id_cols = c(model, gm_dt),
  ...
  # ) |>
  ggplot2::ggplot(ggplot2::aes(x = dplyr::dense_rank(gm_dt), y = s0)) +
  # ggplot2::facet_wrap(ggplot2::vars(category, rowname), scales = "free_x", nrow = 4) +
  ggplot2::geom_hline(yintercept = 0, color = "red", linetype = 2) +
  ggplot2::geom_line(linewidth = 1) +
  ggplot2::scale_color_viridis_d() +
  ggplot2::scale_x_continuous(
    ...
    breaks =
      dynamic_xg_weights_testing |>
      dplyr::select(gm_dt) |>
      dplyr::mutate(rank = dplyr::dense_rank(gm_dt)) |>
      dplyr::group_by(lubridate::year(gm_dt), lubridate::month(gm_dt)) |>
      dplyr::filter(
        lubridate::month(gm_dt) %in% c(10),
        gm_dt == min(gm_dt)
      ) |>
      dplyr::pull(rank),
    minor_breaks = NULL,
    labels =
      c("22-23", "23-24", "24-25", "25-26")
      # dynamic_xg_weights_testing |>
      # dplyr::select(gm_dt) |>
      # dplyr::group_by(lubridate::year(gm_dt), lubridate::month(gm_dt)) |>
      # dplyr::filter(
      #   lubridate::month(gm_dt) %in% c(10, 12, 2),
      #   gm_dt == min(gm_dt)
      # ) |>
      # dplyr::mutate(label = format(gm_dt, "%b %Y")) |>
      # dplyr::pull(label)
  ) +
  ggplot2::scale_y_continuous("Coefficient") + #, limits = c(-1.1, 0.6)) +
  ggplot2::labs(
    title = "5-on-5 Expected Goal Intercept Coefficient Trends, Regular Season 2022-23 through 2025-26",
    subtitle = "Logistic Ridge Regression Model",
    caption = "Data via NHL"
  ) +
  # ggplot2::theme_minimal() +
  ggplot2::theme(
    legend.position = "bottom",
    panel.spacing.x = ggplot2::unit(2, "line")
